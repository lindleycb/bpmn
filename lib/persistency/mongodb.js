...
Persistency.prototype.close = function(done) {
    ...
    */
    ...
    if (client) {
        client.close(true, function(error) {
            self._resetConnection();
            done(error);
        });
    } else {
...
Persistency.prototype._execute = function(dbCall, argument, done) {
    ...
    if (connection) {
        ...
        if (waitingForConnection) {
            ...
            self._connect(function(error, client) {
 ...
 * @private
 */
Persistency.prototype._getClient = function() {
    return uniqueDbClients[this.uri];
};
 ...
 * @private
 */
Persistency.prototype._setClient = function(client) {
    uniqueDbClients[this.uri] = client;
};
